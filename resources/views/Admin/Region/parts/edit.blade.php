...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('regions.update',$region->id)}}">
    @csrf
    @method('PUT')
    <div class="row mt-0">
        <div class=" mb-2 row col-12 col-lg-6 mt-0 ml-3">
            ...
            <select name="city_id" class="form-control form-control-solid select2">
                ...
                @foreach($cites as $city)
                    <option  value="{{$city->id}}" {{$region->city_id == $city->id?'selected':''}}>{{$city->name_en}} </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="d-flex flex-column mb-2 fv-row col-md-12 col-sm-12 mt-0">
            ...
            <input type="text" class="form-control form-control-solid " placeholder="name (fr)"
                   name="name_fr" value="{{$region->name_fr}}"/>
        </div>
        <!--end::Input group-->
