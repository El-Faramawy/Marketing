<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.36/pdfmake.min.js?{{time()}}"></script>
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.36/vfs_fonts.js?{{time()}}"></script>
    ...
    $(function () {
        var table = $("#exportexample").DataTable({
            ...
            "language": {
                ...
                "sLengthMenu": "{{__('show')}} _MENU_ {{__('record')}}",
                "sZeroRecords": "{{__('no records')}}",
                "sInfo": "{{__('Total records')}} _TOTAL_ {{__('record')}}",
                // "sInfo": "اظهار _START_ الى  _END_ من _TOTAL_ سجل",
                "sInfoEmpty": "{{__('no records')}}",
                "sInfoFiltered": "{{__('Search')}}",
                "sSearch": "{{__('Search')}} :    ",
                "oPaginate": {
...
// $('#product-model').modal('hide')
    ...
    $(document).on('click', '.delete', function () {
        ...
        swal.fire({
            ...
            text: "{{__('This record would be deleted ?')}}",
            icon: "warning",
                ...
                success: function (data) {
                    ...
                    window.setTimeout(function() {
                        ...
                        if (data.code == 200){
                            ...
                            my_toaster("{{__('there is an error')}}",'error')
                        }
    ...
    $(document).on('click', '#multiDeleteBtn', function () {
        ...
        if(allVals.length <=0)
        {
            my_toaster("{{__('No selected records to delete')}}",'warning')
        }
        else {
            swal.fire({
                title: "{{__('Are You Sure ?')}}",
